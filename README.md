...
**Shared foundation plus Harbor synthetic vertical slice.** The repository now
contains an accessible Next.js Harbor route and a modular FastAPI workflow for
bounded synthetic requests, explainable matching, coordinator triage, capacity
reservation, volunteer plan approval, fulfillment, audit, and metrics. It reuses
the foundation's authentication, organization isolation, roles, and typed
TypeScript client. There is no deployed service, real user, partner, aid
provision, or measured field outcome. See [GitHub issue
#2](https://github.com/grey-ghost-1/zion-software-initiative/issues/2).

Harbor is the only implemented initiative prototype. These directions remain
unimplemented:
